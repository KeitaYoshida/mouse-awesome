...
「bDisableMouseAcceleration=False」になってしまう問題を自動的に検知し、強制的にオフ(True)へ書き換える
処理を行うためのものです。
	•	Fortnite側の仕様やアップデートにより、マウス加速が勝手にオンになるケースを防止し、安定したマウス操作を確保します。
	•	完全に裏側で動作し、加速を検知した場合のみ通知してくれます。

なお、本スクリプト（VBScript）を採用している理由として、以下の利点があります。
	1.	Windows標準の機能(WSH) で動くため、追加インストール不要。
...
# 3. 設計・危険性への配慮
	...
	4.	スクリプトの終了方法
	•	本スクリプトは標準だとタスクトレイにアイコンを表示しないため、手動で終了する場合は
    タスクマネージャ→wscript.exe プロセスを終了 させる必要があります。
